...
public class Bai2 {
	public static void main(String [] args) {
		...
		Date date = null;
		try {
			date = dateFormat1.parse(s3 + '-' + s2 + '-' + s1);
		} catch (ParseException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		Date currentDate = new Date();
		Date sCurrentDate = null;
		try {
			sCurrentDate = dateFormat1.parse(dateFormat1.format(currentDate));
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		System.out.println("Ngày nhập: "+dateFormat2.format(date));
		System.out.println("Ngày hôm nay: " + dateFormat2.format(currentDate));
		
		long  x = ((date.getTime() - sCurrentDate.getTime())/86400000);
		if (x == 0)
